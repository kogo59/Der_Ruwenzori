BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Der_Ruwenzori"
TITLE_NAME = "Der Ruwenzori"
...
$(BUILD)/epub/$(OUTPUT_FILENAME).epub: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(CSS_FILE_KINDLE) $(IMAGES) \
	...
	mkdir -p $(BUILD)/epub
	$(PANDOC) $(ARGS) --from markdown+raw_html+fenced_divs+fenced_code_attributes+bracketed_spans+multiline_tables --to epub+raw_html --resource-path=$(IMAGES_FOLDER) --epub-cover-image=$(COVER_IMAGE) -o $@  $(PREFACE_EPUB) $(CHAPTERS)
